...
# TEST USING 1,000, 3,000, 5,000, 10,000
...
x_train_1h <- one_hot(train$x, max_var + 3)
x_test_1h <- one_hot(test$x, max_var + 3)
dim(x_train_1h)
nnzero(x_train_1h) / (25000 * (max_var + 3))
...
# Find best lambda
model <- nn_module(
    initialize = function(input_size = max_var + 3) {
...
plot(fitted) 
print(fitted$records$metrics$valid[[10]]$acc)